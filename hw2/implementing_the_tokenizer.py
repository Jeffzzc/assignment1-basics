    ...
    def merge_tokens(self, token_bytes: bytes) -> List[bytes]:
        ...
        while True:
            ...
            for i in range(len(token_list_bytes) - 1):
                ...
                if current_pair_to_check in self._merges_priority_map:
                    current_merge_priority_index = self._merges_priority_map[current_pair_to_check]
                    # possible_merges.append((priority_index, i))
                    # 判断当前对是否是最佳合并：
                    # 1. 如果这是第一个找到的有效合并对
                    # 2. 或者当前对的优先级更高 (索引值更小)
                    # 3. 或者优先级相同，但当前对更靠左 (位置索引更小)
                    if best_pair_merge_priority_index == -1 or current_merge_priority_index < best_pair_merge_priority_index or (current_merge_priority_index == best_pair_merge_priority_index and i < best_pair_position):
                        best_pair_merge_priority_index = current_merge_priority_index
            ...
            merged_token_bytes = b1_to_merge + b2_to_merge
            # 替换原列表中的两个元素为合并后的新元素
            token_list_bytes = token_list_bytes[:best_pair_position] + [merged_token_bytes] + token_list_bytes[best_pair_position + 2:]
    ...
    def encode(self, text: str) -> List[int]:
        ...
        for i, segment_str in enumerate(segments):
            ...
            if segment_bytes in self._special_tokens_bytes:
                ...
                pre_tokenized_tokens = self.pre_tokenize(segment_str)
